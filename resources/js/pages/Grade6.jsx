...
const Grade6 = () => {
  ...
  // දැනට Play වන වීඩියෝව තබා ගැනීමට (Default එකක් ලෙස පළමු වීඩියෝව පසුව සෙට් වේ)
  const [activeVideo, setActiveVideo] = useState("");

  // Laravel Backend URL එක (ඔයාගේ Port එක 8000 නම්)
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        ...
        // පැමිණෙන සියලුම දත්ත වලින් 'Grade 6' සහ 'Active' ඒවා පමණක් පෙරරා ගැනීම
        const grade6Data = data.filter(item => item.grade === '6' && item.status === 'Active');
        
        setTopicsData(grade6Data);

        // වීඩියෝ ටැබ් එක සඳහා මුලින්ම පෙන්වීමට වීඩියෝවක් ඇත්නම් එය activeVideo එක ලෙස තෝරා ගැනීම
        const firstVideo = grade6Data.find(item => item.type === 'video');
        if (firstVideo) {
  ...
  // PDF ෆයිල් එකක් බ්‍රවුසරයෙන් open කරගැනීමට ලින්ක් එක සකසන ෆන්ක්ෂන් එක
  const handleDownload = (filePath, typeName) => {
    if (filePath) {
      // Laravel public storage එකට ලින්ක් කිරීම
  ...
  // YouTube URL එක Embed URL එකක් බවට පත් කිරීමේ ආරක්ෂිත ක්‍රමවේදයක්
  const formatEmbedUrl = (url) => {
    if (!url) return "";
    if (url.includes("embed")) return url;
    // සාමාන්‍ය watch?v= ලින්ක් එකක් නම් embed එකක් බවට හරවයි
    const regExp = /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/;
    const match = url.match(regExp);
    return (match && match[2].length === 11) ? `https://www.youtube.com/embed/${match[2]}` : url;
  };

  // Tab එක අනුව දත්ත වෙන් කරගැනීම
    ...
    <div className="min-h-screen bg-[#fafbfc] font-sans text-[#071835] selection:bg-[#b5cbf0] selection:text-[#010813] antialiased">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 xl:col-span-9 space-y-5">
          ...
          {/* loading සහ Error හැන්ඩ්ල් කිරීම */}
          {loading && (
          ...
          {/* දත්ත කිසිවක් නැති විට පෙන්වීමට */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
              {tutesList.map((topic, index) => (
                ...
                >
                  ...
                  <button 
                    onClick={() => handleDownload(topic.file_path, 'Tute PDF')}
          ...
          {/* ---------------- SHORT NOTES TAB CONTENT ---------------- */}
          {!loading && !error && activeTab === 'notes' && notesList.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
              {notesList.map((topic, index) => (
                <div
                  key={topic.id}
                  className="group bg-white border border-[#b5cbf0]/30 hover:border-[#10b981]/40 p-5 rounded-2xl shadow-[0_4px_16px_rgba(7,24,53,0.02)] hover:shadow-[0_12px_24px_rgba(16,185,129,0.08)] transition-all duration-300 flex flex-col justify-between gap-5 relative overflow-hidden"
                >
                  ...
                  <button 
                    onClick={() => handleDownload(topic.file_path, 'Short Note PDF')}
                    className="w-full py-2.5 bg-gradient-to-r from-[#071835] to-[#122e5c] hover:from-[#10b981] hover:to-[#059669] text-white font-mono font-bold text-[11px] uppercase tracking-wider rounded-xl transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-2 shadow-sm"
                  >
              ...
              <div className="xl:col-span-2 space-y-4">
                <div className="relative pb-[56.25%] h-0 rounded-2xl overflow-hidden shadow-sm bg-black border border-[#b5cbf0]/20">
                  <iframe
                    ...
                    src={formatEmbedUrl(activeVideo)}
                ...
                {videosList.map((topic) => (
                  <div 
                    ...
                    className={`p-4 rounded-xl border transition-all duration-200 flex flex-col justify-between gap-3 ${
                      activeVideo === topic.video_url
                        ? 'bg-[#b5cbf0]/15 border-[#5d81bd] shadow-sm'
                        : 'bg-white border-[#b5cbf0]/30 hover:border-[#5d81bd]/40'
                    }`}
                  >
                    <div>
                    ...
                    <button
                      onClick={() => setActiveVideo(topic.video_url)}
                      className={`w-full py-2 rounded-lg font-mono font-bold text-[10px] uppercase tracking-wider transition-colors flex items-center justify-center gap-1.5 ${
                        activeVideo === topic.video_url
                          ? 'bg-[#071835] text-white'
                          : 'bg-gray-100 text-gray-700 hover:bg-[#5d81bd] hover:text-white'
                      }`}
                    >
